...
const WaiterEdit = () => {
  ...
  const getOneWaiter = async (id) => {
    const w = await axios.get('http://192.168.0.3:4000/api/waiter/' + id);
  ...
  const updateOneWaiter = async () => {
    await axios.put('http://192.168.0.3:4000/api/waiter/' + id, {
        qty: qty || oldQty,
  ...
  const deleteWaiter = async () => {
    await axios.delete('http://192.168.0.3:4000/api/waiter/' + id);
    window.location.href='http://192.168.0.3:3000/waiter';
  }
