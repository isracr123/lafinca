...
const WaiterAdminEdit = () => {
  ...
  const getOneWaiter = async (id) => {
    const w = await axios.get('http://192.168.0.10:4000/api/waiterAdmin/' + id);
  ...
  const updateOneWaiter = async () => {
    await axios.put('http://192.168.0.10:4000/api/waiterAdmin/' + id, {
        qty: qty || oldQty,
  ...
  const deleteWaiter = async () => {
    await axios.delete('http://192.168.0.10:4000/api/waiterAdmin/' + id);
    window.location.href='http://192.168.0.10:3000/pos/waiterAdmin';
  }
...
export default WaiterAdminEdit