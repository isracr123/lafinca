...
const RightSide = (props) => {
  ...
  const addTotal = () => {
    if (totalLength === 0){
      axios.post('http://192.168.0.243:4000/api/total', {
        total: total,
        pay: pago,
        change: cambio, 
      })
  ...
  const newTotal = async (id) => {
    await axios.put('http://192.168.0.243:4000/api/total/' + id, {
      total: total,
      pay: pago,
      change: cambio, 
    });
  ...
  const deleteCart = async (id, qty) => {
    await axios.delete('http://192.168.0.243:4000/api/cart/' + id);
  ...
  const addSum = async (id, sum, qty) => {
    if (props.stopSum === 0){
      ...
      await axios.put('http://192.168.0.243:4000/api/cart/'+ id,{
      sum: sum + 1,
      });
      props.changeIdQty(id);
      ...
      <div className='bg-white rounded-3xl flex flex-col h-full shadow'>
        <div className='flex-1 flex flex-col overflow-auto'>
          ...
          <div className='select-none h-auto w-full text-center pt-3 pb-4 px-4'>
            ...
            onClick={()=> {
              const function1 = window.location.href='http://192.168.0.243:3000/pos/receipt';
