...
const WaiterAdmin = () => {
  ...
  const getWaiters = async () => {
    const w = await axios.get('http://192.168.0.10:4000/api/waiterAdmin');
    setWaiter(w.data);
  ...
  /*Create Waiter data*/
  const createWaiter = () => {
    axios.post('http://192.168.0.10:4000/api/waiterAdmin', {
                qty: qty,
                products: value
    });
    setRefresh(true);
  ...
  const deleteAllWaiter = async () => {
    await axios.delete('http://192.168.0.10:4000/api/waiterAdmin');
  }
